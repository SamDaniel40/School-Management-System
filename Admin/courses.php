...
if (isset($_POST['submit'])) {
  ...
  if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["thumbnail"]["tmp_name"], $target_file)) {
      sqlsrv_query($db_conn, "INSERT INTO tbl_Courses (Name, Category, Duration,Image, Date) VALUES (?,?,?,?)", array($name, $category, $duration, $image, $today)) or die(sqlsrv_errors());
          ...
          <div class="table-responsive bg-white">
            <table class="table table-bordered">
              ...
              <tbody>
                <?php
                $count = 1;
                $curse_query = sqlsrv_query($db_conn, 'SELECT * FROM tbl_Courses');
                while ($course = sqlsrv_fetch_object($curse_query)) { ?>
                  <tr>
                    <td><?= $count++ ?></td>
                    <td><img src="../dist/uploads/<?= $course->Image ?>" height="100" alt="<?= $course->Name ?>"
                        class="border">
                    </td>
                    <td><?= $course->Name ?></td>
