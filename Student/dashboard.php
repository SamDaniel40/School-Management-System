    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <?php
          ...
          $current_year = date('Y');
          $current_date = date('d');
          $sql = "SELECT * FROM tbl_Attendance WHERE AttendanceMonth = ? AND year(CurrSession) = ? AND StdId = ?";

          $query = sqlsrv_query($db_conn, $sql,array($current_month,$current_year,$std_id));

          $row = sqlsrv_fetch_object($query);
          $attendance = unserialize($row->AttendanceValue) ;
          // echo '<pre>';
          // print_r($attendance);
          // echo '</pre>';
        if(isset($_POST['sign-in'])){
          ...
          $current_month = strtolower(date('F'));
          $sql = "UPDATE tbl_Attendance SET AttendanceValue = ? WHERE AttendanceMonth = ? AND StdId = ?";

          sqlsrv_query($db_conn,$sql,array($att_data,$current_month,$std_id)) or die('DB error');;
        }
        if(isset($_POST['sign-out'])){
          ...
          $att_data = serialize($attendance);
          $current_month = strtolower(date('F'));
          $sql = "UPDATE tbl_Attendance SET AttendanceValue = ? WHERE AttendanceMonth = ? AND std_id = ?";
