...
if (isset($_POST['form_submitted'])) {
    ...
    $query = sqlsrv_query($db_conn, "INSERT INTO tbl_Posts (Title, Type,Description, Status, Author,Parent) VALUES (?,'Payment','',?,?,0)",array($title,$status,$user_id));
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            $sql = "SELECT m.MetaValue as Month FROM tbl_Posts as p JOIN tbl_MetaData as m ON p.Id = m.ItemId WHERE p.Type = 'Payment' AND p.Author = ? AND m.MetaKey = 'Month' AND year(p.PublishDate) = 2023";

                            $query = sqlsrv_query($db_conn, $sql,array($user_id));
                            $paid_fees = [];
